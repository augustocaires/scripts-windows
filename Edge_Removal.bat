...
$CMD = "$env:systemroot\system32\conhost.exe $MIN" # minimize prompt - see Terminal issue #13914
...
for /f %%E in ('"prompt $E$S& for %%e in (1) do rem"') do echo;%%E[2t 2>nul & rem minimize prompt
...
if not defined {var} (set {reg}=& set "%~3="& exit /b) else if %2=="" set "{var}=%{var}:*)    =%"& rem
...
write-host -nonew -fore green -back black "`n EDGE REMOVED!"; write-host -back black
